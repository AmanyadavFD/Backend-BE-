...
// Find a movie by id and delete
async function deleteMovie(movieId) {
  try {
    const deleteMovie = await Movie.findByIdAndDelete(movieId);
    console.log("This movie deleted from dataBase ", deleteMovie);
  } catch (error) {
    console.log("Error in Deleting Movie ", error);
  }
}
deleteMovie("66ba894fc2af8e64063905cf");

async function deleteMovieFromDb(movieTitle) {
  try {
    const deleteMovie = await Movie.findOneAndDelete({ title: movieTitle });
    console.log("Deleted movie by title", deleteMovie);
  } catch (error) {
    console.log("Error in Deleting Movie ", error);
  }
}
