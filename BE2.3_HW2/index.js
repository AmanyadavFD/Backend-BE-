...
//66dadd175f82b4b79dea9094

async function updateHotelById(hotelId, updateData) {
  try {
    const updatedData = await Hotel.findByIdAndUpdate(hotelId, updateData, {
      new: true,
    });
...
updateHotelById("66dadd175f82b4b79dea9094", { checkOutTime: "11 AM" });
...
async function updateHotelByPhoneNumber(hotelPhoneNum, updatDate) {
  try {
    const updatedData = await Hotel.findOneAndUpdate(
      { phoneNumber: hotelPhoneNum },
      updatDate,
      { new: true }
    );
    console.log(updatedData);
