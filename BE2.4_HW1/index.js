...
async function deleteRestaurantById(restaurantID) {
  try {
    ...
    console.log("Deleted the Restaurant from the DB: ", deleteRestaurant);
  } catch (error) {
    console.log("Error in Deleting Restaurant ", error);
  }
}
deleteRestaurantById("66d80c17c5ef368883e21729");

async function deleteRestaurantByName(restaurantName) {
  try {
    const deleteByName = await Restaurant.findOneAndDelete({
      name: restaurantName,
    });
    console.log("Delete the Restaurant by the name: ", deleteByName);
  } catch (error) {
    console.log(error);
  }
}
